...
## 📋 Revit 參數設定與 Excel 裝修表結合指南 (新手必讀)
 
為了讓外掛功能與您的專案參數完美對接，請參照以下設定說明：
 
### 1. Revit 房間 (Room) 參數對接
*   **地板、牆面、天花板裝修**：外掛直接讀寫 Revit `Room` 原生的內建參數：
    *   地板裝修 (Floor Finish) ➡️ 對應 `ROOM_FINISH_FLOOR`
    *   牆面裝修 (Wall Finish) ➡️ 對應 `ROOM_FINISH_WALL`
    *   天花板裝修 (Ceiling Finish) ➡️ 對應 `ROOM_FINISH_CEILING`
*   **踢腳板裝修**：Revit 無原生踢腳板參數，外掛會優先尋找專案中名為 **`"踢腳板"`**、**`"踢腳板裝修"`** 或 **`"Baseboard"`** 的房間自訂參數。若專案中尚未建立該參數，使用者仍可在外掛 DataGrid 中填寫裝修代號，外掛會保留在記憶體中以支援一鍵生成實體踢腳板。
 
### 2. 雲端 Excel 裝修對照表設計 (cm 格式)
可透過「房間裝修配置」介面中的「📋 下載範例 CSV」下載公分 (cm) 規格的範本，並將其上傳至 Google Drive 後「發布為 CSV」，以作為外掛的雲端同步連結。
 
*   **欄位定義與單位說明**：
    *   `裝修代號`：如 `F1`, `W1`, `C1`, `B1`。
    *   `類型`：必須為 `Floor` (地坪)、`Wall` (牆面)、`Ceiling` (天花板) 或 `Baseboard` (踢腳板)。
    *   `材料名稱`：對應專案中的 Revit 材料名稱。
    *   `總厚度(cm)`：裝修的總厚度，外掛在 Revit 中會**自動乘以 10 轉換為公釐 (mm)** 以建立對應實體。
    *   `高度或預留(cm)`：
        *   對 `Floor` (地坪) 而言，高程偏移量從 0 開始（依您的 CSV 設定，為 0 則不做偏移）。
        *   對 `Ceiling` (天花板) 而言，代表天花板上方需要預留的施工空間 (如 10cm)。
        *   對 `Baseboard` (踢腳板) 而言，代表**踢腳板的物理高度** (如 10cm 或 120cm)。
