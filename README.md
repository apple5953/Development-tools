...
| 按鈕名稱 | 執行指令 | 詳細功能說明 |
| :--- | :--- | :--- |
| **📐 樓板貼房間** | `Cmd_FloorSnapToRoom` | **【樓板邊界自動貼齊】** 自動搜尋樓板對應的房間，並將其草圖邊緣直線貼齊房間的完成面 (Finish Boundary) 且自動修剪拐角，維持草圖閉合。 |

---

## 🚀 新手快速安裝指南 (3 分鐘上手)

本外掛全面支援當前使用者免管理員權限 (Per-User) 部署，安裝時**完全不需要輸入系統管理員 UAC 密碼**，並完美相容 **Revit 2024、2025、2026**。請提供給同仁以下兩種方式之一進行安裝：

### 📌 方式一：綠色免安裝一鍵部署 (推薦，可避開瀏覽器防毒阻擋)
1.  從 GitHub 下載最新的綠色免安裝包 **`DevelopmentTools_v{Version}.zip`** (例如 `DevelopmentTools_v1.3.0.zip`)。
2.  **【📢 重要防呆】**：請務必先將壓縮包**完整解壓縮**，切勿在壓縮檔內部直接執行！
3.  進入解壓縮後的資料夾，滑鼠雙擊執行 **`install.bat`**。
4.  當視窗提示安裝完成後，按任意鍵關閉視窗，即自動完成檔案複製與增益集註冊。

### 📌 方式二：手動安裝安裝包 (EXE)
1.  下載並雙擊執行 **`DevelopmentTools_Setup.exe`**。
2.  本安裝程式已調整為最低權限要求 (Lowest Privilege)，直接點選下一步即可安裝完成，**全程不會彈出管理員密碼要求**。
...
3.  請務必點選 **`永遠載入 (Always Load)`**，以確保每次啟動 Revit 時外掛能正常運作。

### 📌 步驟 3：Google 登入驗證與自動開通
1.  點選 Revit 上方 `Development tools` 頁籤，點選面版中的任意功能（如 Google 登入或檢查更新）。
2.  外掛會彈出提示並自動打開網頁瀏覽器，引導您進行 Google 帳號授權驗證。
